...
| Управление             | Команды плагинов в обычных комнатах | Приватная control-room с диагностикой, управлением подключением, питанием ноды и LoRa TX |
...
| `node reboot confirm` / `node shutdown confirm`               | Перезагрузить или выключить локальную ноду                        |
| `disconnect` / `connect`                                      | Освободить подключение к ноде или снова подключить мост           |
| `lora tx status` / `lora tx on`                               | Проверить или включить передачу LoRa                              |
| `lora tx off confirm`                                         | Отключить передачу LoRa, сохранив приём эфира                     |
...
`disconnect` не останавливает контейнер и Matrix-часть моста. Автоматическое
переподключение и проверки соединения приостанавливаются до команды `connect`,
поэтому освободившееся подключение можно использовать из приложения или
браузера. После `node shutdown confirm` мост также остаётся в этом состоянии,
чтобы не пытаться подключаться к выключенной ноде. Команда `lora tx off confirm`
меняет штатный параметр Meshtastic `LoRaConfig.tx_enabled`; приём LoRa остаётся
включённым, а применение настройки может кратковременно перезапустить ноду.
